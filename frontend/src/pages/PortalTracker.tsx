...
This is AI-generated legal guidance for advisory purposes only. Please consult a qualified lawyer for your specific case.`;
  ...
  return (
    <ScreenShell>
      <StickyHeader title={t("portalTrackerTitle")} showBack onBack={() => navigate(-1)} showLanguagePill />
      <div className="px-5 pt-4 pb-8 space-y-5 animate-fade-in-up">
        ...
        <div>
          ...
          {recommendation && recommendation !== activePortal && (
            ...
            >
              <Sparkles size={12} /> {t("portalAiSuggest")} {PORTALS.find((p) => p.key === recommendation)?.name}
            </button>
          )}
        </div>
          ...
          <div className="mt-4">
            <div className="flex items-center justify-between text-xs mb-1.5">
              ...
              <span className="text-muted-foreground">
                {completedCount}/4 {t("steps")} · {progressPct}%
              </span>
            </div>
        ...
        <div className="space-y-3">
          ...
          <StepCard
            id="file"
            icon={<Pencil size={18} />}
            title={t("fileComplaintTitle")}
            description={t("fileComplaintDesc")}
          ...
          >
            <div className="flex flex-wrap gap-2">
              <Button leftIcon={<Sparkles size={14} />} onClick={generateDraft} className="h-10 text-xs px-4">
                {portalState.draft ? t("regenerateDraft") : t("generateDraftButton")}
              </Button>
              {portalState.draft && (
                <>
                  <Button variant="secondary" onClick={() => setEditingDraft((v) => !v)} className="h-10 text-xs px-4">
                    {editingDraft ? t("doneEditing") : t("edit")}
                  </Button>
          ...
          <StepCard
            ...
            title={t("payFilingFeeTitle")}
            description={`${t("applicableFilingFee")}: ${portal.fee}`}
          ...
          >
            <div className="ls-card p-3 text-xs">
              <p>
                <strong>{t("fee")}:</strong> {portal.fee}
              </p>
            ...
            <div className="mt-3 flex gap-2">
              ...
              >
                <ExternalLink size={13} /> {t("payOnPortal")}
              </a>
              <Button variant="secondary" className="h-10 text-xs px-4" onClick={() => toggleStep("pay")}>
                {portalState.steps.pay ? t("markUnpaid") : t("ivePaid")}
              </Button>
            </div>
          </StepCard>
          ...
          >
            <div className="ls-card p-3 text-xs space-y-1.5">
              <StatusLine label={t("statusFiledOnPortal")} ok={portalState.steps.file} />
              <StatusLine label={t("statusFeePaid")} ok={portalState.steps.pay} />
              <StatusLine label={t("statusAckReceived")} ok={portalState.steps.pay && portalState.steps.file} />
              <StatusLine label={t("statusHearingResolution")} ok={portalState.steps.track} />
            </div>
            <div className="mt-3 flex gap-2">
              <a
                href={portal.url}
              ...
              <Button variant="ghost" className="h-10 text-xs px-4" onClick={() => toggleStep("track")}>
                {portalState.steps.track ? t("reopen") : t("markResolved")}
              </Button>
            </div>
          </StepCard>
        </div>
