...
class DocumentMemoryService:
    ...
    def ingest(
        ...
        """
        Full ingestion pipeline. Returns the new document UUID or a stub dict if Supabase is offline.
        Raises on critical failure.
        """
        sb = get_supabase_admin()
        if sb is None:
            print("[DOCMEM] Supabase not configured — ingest returned stub dict.")
            return {"doc_id": "local-only", "chunks": 0, "warning": "Supabase not configured"}
    ...
    def retrieve(
        ...
        sb = get_supabase_admin()
        if sb is None:
            print("[DOCMEM] Supabase not configured — retrieve returned empty string.")
            return ""
    ...
    def list_documents(self, user_id: str, session_id: Optional[str] = None) -> List[dict]:
        ...
        sb = get_supabase_admin()
        if sb is None:
            print("[DOCMEM] Supabase not configured — list_documents returned empty list.")
            return []
        try:
            q = (
    ...
    def delete_document(self, doc_id: str, user_id: str) -> bool:
        ...
        sb = get_supabase_admin()
        if sb is None:
            print("[DOCMEM] Supabase not configured — delete_document returned False.")
            return False
        try:
            result = (
                sb.table("user_documents")
    ...
    def _persist(
        ...
        sb = get_supabase_admin()
        if sb is None:
            print("[DOCMEM] Supabase not configured — _persist ignored.")
            return "local-only"
    ...
    def _search(
        ...
        try:
            ...
            sb = get_supabase_admin()
            if sb is None:
                print("[DOCMEM] Supabase not configured — _search returned empty list.")
                return []
