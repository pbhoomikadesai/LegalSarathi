...
class FastTranslator:
    ...
    def translate(self, text: str, source_lang: str, target_lang: str = "en") -> str:
        # Normalize language codes (e.g., 'hi-IN' -> 'hi', 'en-US' -> 'en')
        src = source_lang.split("-")[0].split("_")[0].lower() if source_lang else "auto"
        tgt = target_lang.split("-")[0].split("_")[0].lower() if target_lang else "en"
        
        if src == tgt:
            return text
        try:
            return self.GoogleTranslator(source=src, target=tgt).translate(text)
