...
class SummarizationService:
    ...
    def maybe_summarize(self, session_id: str, language: str = "hi") -> None:
        ...
        try:
            ...
            if sb is None:
                print("[SummarizationService] Supabase not configured — maybe_summarize ignored.")
                return
    ...
    async def _run(self, session_id: str, language: str) -> None:
        ...
        try:
            sb = get_supabase_admin()
            if sb is None:
                print("[SummarizationService] Supabase not configured — _run ignored.")
                return
    ...
    def get_summary(session_id: str) -> str:
        ...
        try:
            sb = get_supabase_admin()
            if sb is None:
                print("[SummarizationService] Supabase not configured — get_summary returned empty string.")
                return ""
            result = (
                sb.table("chat_sessions")
                .select("summary")
