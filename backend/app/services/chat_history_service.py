...
class ChatHistoryService:
    ...
    def create_session(
        ...
        if sb is None:
            print("[ChatHistoryService] Supabase not configured — create_session ignored.")
            return {}
        expires_at = (
    ...
    def list_sessions(
        ...
        sb = get_supabase_admin()
        if sb is None:
            print("[ChatHistoryService] Supabase not configured — list_sessions returned empty.")
            return {"sessions": [], "page": page, "page_size": page_size}
    ...
    def rename_session(self, user_id: str, session_id: str, new_title: str) -> dict:
        ...
        sb = get_supabase_admin()
        if sb is None:
            print("[ChatHistoryService] Supabase not configured — rename_session ignored.")
            return {}
        result = (
            sb.table("chat_sessions")
    ...
    def toggle_pin(self, user_id: str, session_id: str, pin: bool) -> dict:
        ...
        sb = get_supabase_admin()
        if sb is None:
            print("[ChatHistoryService] Supabase not configured — toggle_pin ignored.")
            return {}
        update = {
            "is_pinned": pin,
    ...
    def delete_session(self, user_id: str, session_id: str) -> bool:
        ...
        sb = get_supabase_admin()
        if sb is None:
            print("[ChatHistoryService] Supabase not configured — delete_session ignored.")
            return False
        result = (
            sb.table("chat_sessions")
            .delete()
    ...
    def get_messages(
        ...
        sb = get_supabase_admin()
        if sb is None:
            print("[ChatHistoryService] Supabase not configured — get_messages returned empty.")
            return {"messages": [], "session_id": session_id, "page": page, "page_size": page_size}
    ...
    def save_message(
        ...
        sb = get_supabase_admin()
        if sb is None:
            print("[ChatHistoryService] Supabase not configured — save_message ignored.")
            return {}
        data = {
            "id": str(uuid.uuid4()),
            "session_id": session_id,
    ...
    def search_messages(
        ...
        sb = get_supabase_admin()
        if sb is None:
            print("[ChatHistoryService] Supabase not configured — search_messages returned empty.")
            return {"results": [], "query": query, "page": page, "page_size": page_size}
        offset = (page - 1) * page_size
        result = (
            sb.table("chat_messages")
    ...
    def update_session_title_from_first_message(
        ...
        try:
            sb = get_supabase_admin()
            if sb is None:
                print("[ChatHistoryService] Supabase not configured — update_session_title ignored.")
                return
