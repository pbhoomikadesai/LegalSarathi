    ...
    async def process_query(self, text: str, lang: str = "hi", pinned_history: list = None, conversation_history: list = None, doc_context: str = "", session_summary: str = ""):
        ...
        if self._lf:
            try:
                ...
                print(f"[OBSERVABILITY] Langfuse trace start error: {e}")
        ...
        if trace:
            try:
                span_translate = trace.span(name="translation", input=text)
            except Exception as e:
                print(f"[OBSERVABILITY] Langfuse span translation error: {e}")
        ...
        if span_translate:
            try:
                span_translate.end(output=english_text)
            except Exception as e:
                print(f"[OBSERVABILITY] Langfuse span translation end error: {e}")
        ...
        if trace:
            try:
                span_parallel = trace.span(name="parallel_retrieval", input=english_text)
            except Exception as e:
                print(f"[OBSERVABILITY] Langfuse span parallel error: {e}")
        ...
        if span_parallel:
            try:
                span_parallel.end(metadata={"rag_candidates": len(rag_chunks_raw)})
            except Exception as e:
                print(f"[OBSERVABILITY] Langfuse span parallel end error: {e}")
        ...
        if trace:
            try:
                span_rerank = trace.span(
                    name="reranking",
                    input={"query": english_text, "chunks_in": len(rag_chunks_raw)}
                )
            except Exception as e:
                print(f"[OBSERVABILITY] Langfuse span rerank error: {e}")
        ...
        if span_rerank:
            try:
                span_rerank.end(metadata={"chunks_in": len(rag_chunks_raw), "chunks_out": len(rag_chunks)})
            except Exception as e:
                print(f"[OBSERVABILITY] Langfuse span rerank end error: {e}")
        ...
        if trace:
            try:
                span_synthesis = trace.span(
                    name="groq_synthesis",
                    input={"english_text": english_text, "legal_keys": legal_keys, "rag_context": rag_context},
                    metadata={"model": "llama-3.3-70b-versatile"}
                )
            except Exception as e:
                print(f"[OBSERVABILITY] Langfuse span synthesis error: {e}")
        ...
        if span_synthesis:
            try:
                span_synthesis.end(output=buddy_data)
            except Exception as e:
                print(f"[OBSERVABILITY] Langfuse span synthesis end error: {e}")
        ...
        if trace:
            try:
                trace.generation(
                    ...
                    input=english_text,
                    output=json.dumps(buddy_data),
                    usage=buddy_data.get("usage", None)
                )
            except Exception as e:
                print(f"[OBSERVABILITY] Langfuse generation event error: {e}")
        ...
        if trace:
            try:
                span_audit = trace.span(
                    name="citation_audit",
                    input={"answer_text": full_answer_text, "chunks": [c.get("section_ref") for c in rag_chunks]}
                )
            except Exception as e:
                print(f"[OBSERVABILITY] Langfuse span audit error: {e}")
        ...
        if span_audit:
            try:
                span_audit.end(output=citation_result, metadata={"citation_score": citation_result["citation_score"]})
            except Exception as e:
                print(f"[OBSERVABILITY] Langfuse span audit end error: {e}")
        ...
        if trace:
            try:
                trace.score(
                    ...
                    value=citation_result["citation_score"]
                )
            except Exception as e:
                print(f"[OBSERVABILITY] Langfuse score error: {e}")
